namespace VulpesX.Models.Default;
 
public partial class SPEDIZIONE_LINGUA : Base 
{
	private string _specod = null!;
	public required string specod { get => _specod; set { if (_specod != value) { _specod = value; NotifyPropertyChanged();} } }
	private string _lincod = null!;
	public required string lincod { get => _lincod; set { if (_lincod != value) { _lincod = value; NotifyPropertyChanged();} } }
	private string? _spedes;
	public string? spedes { get => _spedes; set { if (_spedes != value) { _spedes = value; NotifyPropertyChanged();} } }
	private byte[]? _rv;
	public byte[]? rv { get => _rv; set { if (_rv != value) { _rv = value; NotifyPropertyChanged();} } }
}