...
{
    public partial class CAUCONT_GROUPS
    {
        ...
        public ObservableCollection<CAUCONT>? CausalsList
        {
            ...
            {
                causalsList = value;
                if (!string.IsNullOrWhiteSpace(caucod))
                    SelectedCausal = causalsList?.Where(w => w.caucod == caucod).FirstOrDefault();
                else
                    SelectedCausal = null;
        ...
        public ObservableCollection<PDCGRUPPI>? GroupsList
        {
            ...
            {
                groupsList = value;
                if (!string.IsNullOrWhiteSpace(grpgrp))
                    SelectedGroup = groupsList?.Where(w => w.P1GRUP == grpgrp).FirstOrDefault();
                else
                    SelectedGroup = null;
        ...
        public ObservableCollection<PDCCONTI>? AccountsList
        {
            ...
            {
                accountsList = value;
                if (!string.IsNullOrWhiteSpace(grpgrp) && !string.IsNullOrWhiteSpace(grpcto))
                    SelectedAccount = accountsList?.Where(w => w.P1GRUP == grpgrp && w.P2CONT == grpcto).FirstOrDefault();
                else
        ...
        public ObservableCollection<PDCSOTTO>? SubaccountsList
        {
            ...
            set
            {
                ...
                if (!string.IsNullOrWhiteSpace(grpgrp) && !string.IsNullOrWhiteSpace(grpcto) && !string.IsNullOrWhiteSpace(grpsct))
                    SelectedSubaccount = subaccountsList?.Where(w => w.P1GRUP == grpgrp && w.P2CONT == grpcto && w.P3SOTC == grpsct).FirstOrDefault();
                else
